...
contract ScopeTwoVotingTransferable is ERC20, Initializable {
  ...
  mapping(uint256 => uint256) public _priceVotes;

  struct VoterInfo {
    uint256 votedAmount;
    uint256 votedPrice;
  }
  /// @notice Данные голосования каждого участника
  mapping(address => VoterInfo) public _votingInfo;

  /// @notice Массив голосовавших за цену
  address[] public _votersForPrice;
  ...
  modifier canVoteForPriceChangeAmount(uint256 price) {
    ...
    require(_votingInfo[msg.sender].votedPrice != price, "You have already voted for this price.");
  ...
  function vote(uint256 price) external canVoteForPriceChangeAmount(price) {
    ...
    _votingInfo[msg.sender] = VoterInfo(_balances[msg.sender], price);
  ...
  function endVoting() external checkVotingTime {
    ...
    for (uint256 i = 0; i < _votersForPrice.length; i++) {
      delete _votingInfo[_votersForPrice[i]];
    }
    for (uint256 i = 0; i < _prices.length; i++) {
      delete _priceVotes[_prices[i]];
    }
  ...
  function buy() external payable {
    require(!_votedForChange[msg.sender], "You cannot buy as you participate in a voting.");
    ...
    VoterInfo storage votingInfo = _votingInfo[msg.sender];
    if (votingInfo.votedPrice != 0) {
      votingInfo.votedAmount += tokensAfterFee;
      _priceVotes[votingInfo.votedPrice] += tokensAfterFee;
    }
  ...
  function sell(uint256 tokensAmount) external {
    require(!_votedForChange[msg.sender], "You cannot sell as you participate in a voting.");
    ...
    VoterInfo storage votingInfo = _votingInfo[msg.sender];
    if (votingInfo.votedPrice != 0) {
      votingInfo.votedAmount -= tokensAmount;
      _priceVotes[votingInfo.votedPrice] -= tokensAmount;
    }
  ...
  function transfer(address to, uint256 amount) public override returns (bool) {
    require(!_votedForChange[msg.sender], "You cannot transfer as you participate in a voting.");

    VoterInfo storage votingInfo = _votingInfo[msg.sender];
    if (votingInfo.votedPrice != 0) {
      votingInfo.votedAmount -= amount;
      _priceVotes[votingInfo.votedPrice] -= amount;
    }

    VoterInfo storage toVotingInfo = _votingInfo[to];
    if (toVotingInfo.votedPrice != 0) {
      toVotingInfo.votedAmount += amount;
      _priceVotes[toVotingInfo.votedPrice] += amount;
    }
  ...
  function transferFrom(address from, address to, uint256 amount) public override returns (bool) {
    require(!_votedForChange[from], "You cannot transfer as you participate in a voting.");

    VoterInfo storage fromVotingInfo = _votingInfo[from];
    if (fromVotingInfo.votedPrice != 0) {
      fromVotingInfo.votedAmount -= amount;
      _priceVotes[fromVotingInfo.votedPrice] -= amount;
    }
  ...
  function clearAll() external onlyOwner {
    for (uint256 i = 0; i < _votersForChange.length; i++) {
      delete _votedForChange[_votersForChange[i]];
    }
    delete _votersForChange;
