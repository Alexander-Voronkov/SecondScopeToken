...
import { ScopeTwoVotingTransferable } from '../typechain-types';
...
describe('ScopeTwoVotingTransferable', function () {
  async function deployScopeTwoVotingTransferableToken() {
    ...
    const typedToken = token as unknown as ScopeTwoVotingTransferable;
  ...
  describe('Voting', function () {
    it('vote for start price voting', async function () {
        ...
        console.log('before cleanup and refund: ', ethers.formatEther(await ethers.provider.getBalance(owner)), 'ETH');

        await token.connect(owner).clearAll();

        console.log('after cleanup and refund: ', ethers.formatEther(await ethers.provider.getBalance(owner)), 'ETH');
    });
  });
});
